...
# Level-weighted monster equipment generation; every wearable is socketable.
const BASE_ITEMS := [
    ...
    {"id":"hide_coat", "name":"旅者皮甲", "slot":"身體", "armor":14.0, "min_level":1, "weight":16.0},
    {"id":"old_boots", "name":"荒徑長靴", "slot":"鞋子", "armor":5.0, "min_level":1, "weight":13.0},
    {"id":"ragged_hood", "name":"灰燼兜帽", "slot":"頭部", "armor":7.0, "min_level":1, "weight":15.0},
    {"id":"wanderer_greaves", "name":"荒原護腿", "slot":"腿部", "armor":9.0, "min_level":1, "weight":15.0},
    {"id":"warden_plate", "name":"衛士殘甲", "slot":"身體", "armor":25.0, "min_level":3, "weight":8.0},
    {"id":"bone_crown", "name":"碎骨冠冕", "slot":"頭部", "armor":13.0, "min_level":3, "weight":8.0},
    {"id":"rift_legguards", "name":"裂隙腿鎧", "slot":"腿部", "armor":17.0, "min_level":3, "weight":8.0},
    {"id":"swamp_treads", "name":"沼澤行者靴", "slot":"鞋子", "armor":9.0, "min_level":3, "weight":8.0},
    {"id":"rift_charm", "name":"微光護符", "slot":"護符", "min_level":2, "weight":8.0},
]
const PREFIXES := [
...
static func _roll_dynamic(profile: String, monster_level: int, elite: bool) -> Dictionary:
    ...
    for source in BASE_ITEMS:
        ...
        elif profile == "warden" and String(item["slot"]) == "身體":
            item["weight"] = float(item["weight"]) * 2.2
        elif profile == "hunter" and String(item["slot"]) == "頭部":
            item["weight"] = float(item["weight"]) * 1.8
        elif profile == "crawler" and String(item["slot"]) == "鞋子":
            item["weight"] = float(item["weight"]) * 1.8
    ...
    var result := {
        "id": String(base["id"]), "base_name":String(base["name"]), "name":display_name,
        "slot":String(base["slot"]), "level":maxi(1, monster_level), "rarity":rarity,
        "prefix":prefix, "suffix":suffix, "color":color, "profile":profile,
    }
    for key in ["weapon_type", "base_damage", "attack_cooldown", "range", "armor"]:
        if base.has(key):
            result[key] = base[key]
    return result
