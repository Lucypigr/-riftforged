...
func _update_enemies(delta: float) -> void:
    ...
    for i in range(enemies.size() - 1, -1, -1):
        ...
        if enemy.global_position.distance_squared_to(player.global_position) > 625.0:
        ...
        speed *= _enemy_speed_multiplier(int(entry["id"]))
        ...
        if String(archetype["behavior"]) == "ranged":
            ...
            if distance > float(archetype["desired_range"]):
                ...
                if float(entry["touch_cd"]) <= 0.0:
                    ...
                    if bool(entry["berserk"]):
                        damage *= 1.25
                    call("_warning_blast", enemy.global_position, maxf(1.4, float(archetype["desired_range"]) + 0.5), damage, 0.48, int(entry["id"]))
                    entry["touch_cd"] = float(archetype["touch_delay"])

        if enemy.velocity.length_squared() > 0.0001:
            var desired_motion := enemy.velocity * delta
            if enemy.test_move(enemy.global_transform, desired_motion):
                var side := direction.rotated(Vector3.UP, PI * 0.5 * (1.0 if int(entry["id"]) % 2 == 0 else -1.0))
                enemy.velocity = side * speed
            enemy.move_and_slide()
        enemies[i] = entry
...
func _enemy_speed_multiplier(_id: int) -> float:
    return 1.0
