...
func _run() -> void:
    ...
    # Explicit test fixture: earn currency through the actual ground pickup path.
    for currency_kind in ["jeweller", "fusing", "chromatic", "refine"]:
        game._spawn_loot_visual(game.player.global_position, {"slot":"通貨", "currency":currency_kind, "name":currency_kind})
