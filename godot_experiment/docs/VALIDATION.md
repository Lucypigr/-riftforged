...
## Gem expansion — 2026-09-21 UTC
Godot `4.7.2.stable.official.ed1daf0bf`: all 11 suites PASS (`tools/verify.py`), including existing 10 regressions. Final targeted `gem_expansion_smoke` rerun PASS, 197 assertions: all 18 new actives enter the hotbar mana gate and lower actual enemy HP; insufficient mana prevents casts; same-frame duplicate rejected; new gems ground-spawn/pickup; all 23 recipes compatible; disconnected and incompatible supports rejected; every added support modifies its compatible skill; duplicate support deduplicated; radius/mana/duration behavior; actual volley and echo projectile counts; construct caps/expiry; fixed travelling range; inventory pause; live guide/search controls.
Export-release Web PASS with no ERROR/WARNING lines. Final PCK 906,600 bytes, gzip 882,687; WASM 39,514,754 bytes, gzip 10,054,758. Node DecompressionStream + WebAssembly.compile PASS; PCK magic GDPC verified. These are artifact/engine checks, not browser gameplay checks.
Supervised preview started successfully, but cloud browser navigation again returned `net::ERR_BLOCKED_BY_CLIENT`. No browser screenshot, browser interaction, physical device, native visual, or frame-rate success claimed. Natural loot progression/balance and new placeholder effect appearance need human playtesting. Article/PoEDB reference access: HTTP 403; recipes are original, not verified article reproductions.
