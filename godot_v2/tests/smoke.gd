...
func _run() -> void:
    ...
    if file == null:
        _fail("Missing static Traditional Chinese OTF")
        return
    var magic := file.get_buffer(4)
    file.close()
    if magic.size() != 4 or magic[0] != 79 or magic[1] != 84 or magic[2] != 84 or magic[3] != 79:
        _fail("Traditional Chinese font is not a static OpenType CFF file")
        return

    var direct_font := FontFile.new()
    var font_error := direct_font.load_dynamic_font(FONT_PATH)
    if font_error != OK:
        _fail("Godot could not load static Traditional Chinese OTF")
        return
    direct_font.allow_system_fallback = false
    for sample in ["攻", "擊", "寶", "石", "裂", "隙", "獸", "菁", "英", "◆", "◇"]:
        if not direct_font.has_char(sample.unicode_at(0)):
            _fail("Static font missing required glyph: %s" % sample)
            return
    ...
    if ui_root.theme == null or ui_root.theme.default_font == null:
        _fail("Clean rebuild UI is not using the shared native font theme")
        return
    ...
    if enemy_label == null or enemy_label.font == null:
        _fail("Enemy Label3D does not use the native Traditional Chinese font")
        return
    ...
    if runtime_font == null or not runtime_font.has_char("攻".unicode_at(0)):
        _fail("Runtime Traditional Chinese font is not active")
        return
    ...
    print("RIFTFORGED_V2_SMOKE_OK static-otf/ui/label3d/camera/combat-pack ready")
    quit(0)
