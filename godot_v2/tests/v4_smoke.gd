...
func _run() -> void:
    ...
    if not _check((hotbar.get("ids", []) as Array)[0] == "ember_bolt", "starter skill missing"):
        return
    game.call("debug_set_mana", 100.0)
    var mana_before := float(game.get("player_mana"))
    game.call("_use_skill", 0)
    var first_mana := float(game.get("player_mana"))
    if not _check(first_mana < mana_before and is_zero_approx(float((game.get("skill_cooldowns") as Array)[0])) and not (hud.skill_buttons[0] as Button).disabled, "mana cast must not start time cooldown"):
        return
    ...
    var charges: Array = game.get("flask_charges")
    var first_charge := int(charges[0])
    ...
    var ordinary: Dictionary = enemies[0]
    var ordinary_id := int(ordinary["id"])
    ...
    if not _check(int((game.get("flask_charges") as Array)[0]) == mini(40, after_life + 2), "same monster refunded twice: %d" % ordinary_id):
        return
    ...
    var boss_floor := int((game.call("debug_v4_state") as Dictionary)["floor"]) + int((game.call("debug_v4_state") as Dictionary)["gems"])
    game.call("_damage_enemy", boss_index, 9999999.0)
    var dead: Dictionary = game.call("debug_v4_state")
    if not _check(bool(dead.get("boss_defeated", false)) and int(game.call("_enemy_index_by_id", boss_id)) < 0 and int(dead["floor"]) + int(dead["gems"]) > boss_floor, "boss did not die uniquely and drop floor reward"):
        return
    ...
    for i in range(ACTIVE.size()):
        ...
        var start := float(game.get("player_mana"))
        game.call("_use_skill", 1)
        if not _check(float(game.get("player_mana")) < start and is_zero_approx(float((game.get("skill_cooldowns") as Array)[1])), "V3 active failed without cooldown: " + ACTIVE[i]):
            return
