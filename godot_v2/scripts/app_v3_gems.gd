...
func _cast_v3_skill(id: String, gem: Dictionary, gear: Dictionary, socket_index: int) -> void:
    ...
    match id:
        "cleave", "ground_slam":
            var reach := 3.5 if id == "cleave" else 5.5
        ...
        "molten_strike":
            _v3_hit_cone(origin, aimed, 2.5, 0.1, damage, supports)
            _v3_projectiles(origin, damage * 0.55, 3, 6.5, supports, 1.4, false, element_color)
        "burning_arrow":
